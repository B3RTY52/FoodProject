...
function cards() {
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/menu').then(data => {
    data.forEach(_ref => {
      ...
      new MenuItem(img, altimg, title, descr, price, '.menu__field .container').renderItem();
    });
  });
}
...
function modal(triggerSelector, modalSelector, modalTimerID) {
  ...
  modal.addEventListener('click', e => {
    if (e.target === e.currentTarget || e.target.getAttribute('data-close') == '') {
...
function slider(_ref) {
  ...
  nextBtn.addEventListener('click', () => {
    if (offset == strToDigits(width) * (slides.length - 1)) {
      offset = 0;
    } else {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2023-06-31');
